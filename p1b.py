...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, output1, output2, label):
        l2_distance = nn.functional.pairwise_distance(output1, output2)
        contrastive_loss = torch.mean(label * torch.pow(l2_distance, 2) + \
...
if args.save:
    ...
    if not args.augmentation:
        plt.savefig('loss_history2.png')
    else:
        plt.savefig('loss_history_with_aug2.png')
...
if args.load:
    ...
    for data in testset_loader:
        ...
        image1, image2, label = Variable(image1).cuda(), Variable(image2).cuda() , Variable(label).cuda()
        output1, output2 = model(image1, image2)
        output = nn.functional.pairwise_distance(output1, output2)
        result = torch.round((-output).add(1))
